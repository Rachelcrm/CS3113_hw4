...
GLuint    g_player_texture_id;// start George looking down
GLuint text_id;
...
void update()
{
    if (!endgame) {
        ...
        while (delta_time >= FIXED_TIMESTEP) {
            ...
            // Check if the enemy reaches the left or right boundary
        ...
        if (die == true || g_game_state.player->get_position().y < -4.0f) {
            DrawText(&g_shader_program, text_id, "YOU LOSE", 0.5, 0, glm::vec3(g_game_state.player->get_position()));
            endgame = true;
        }
        
        else if (enemy1_deact && enemy2_deact && enemy3_deact) {
            LOG("YES");
            DrawText(&g_shader_program, text_id, "YOU WIN", 0.5, 0, glm::vec3(g_game_state.player->get_position()));
            endgame = true;
        }
    }
}
